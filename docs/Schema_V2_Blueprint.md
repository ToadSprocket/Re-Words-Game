...
- Type IDs (int):
  - `leaderboardGameTypeId`, `leaderboardCategoryTypeId`, `userStatusTypeId`, `userIdentityStateTypeId`
...
- index: `{ boardId: 1, leaderboardGameTypeId: 1, score: -1 }`

---

## 4) `game_words`

Purpose: Table that shows the possible words for a game, generated by the creation script, does not tie to any user.
         Will be used for game monitoring dashboard. Can only be read by the dashboard account.
...
- index: `{ boardId: 1, leaderboardGameTypeId: 1, leaderboardCategoryTypeId: 1, score: -1 }`
- index: `{ userId: 1, createdAtUtc: -1 }`
- index: `{ leaderboardGameTypeId: 1, leaderboardCategoryTypeId: 1 }`
...
- unique: `{ leaderboardGameTypeId: 1 }`
